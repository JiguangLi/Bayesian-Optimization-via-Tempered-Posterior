...
This repo contains the official Python implementation of our proposed tempered Bayesian optimization framework described in the paper [Robust bayesian optimization via tempered posteriors](https://arxiv.org/abs/2601.07094). The current codebase implements tempered (or regular) GP surrogate, with generalized expected improvement acquization functions (including both probability of improvement, expected improvement, and more). Our tempering methodlogy can be generalized to other acquization functions as well.
...
- ```project/x04_bo_experiments_*```: driver code for the simulation exercise in Section 6.1 of the paper.
   - ```x04_bo_experiments_gaussian```: Gaussian noise setting
   - ```x04_bo_experiments_gm```:  Gaussian mixture noise setting
   - ```x04_bo_experiments_hs```: heteroscedastic noise
   - ```x04_bo_experiments_t```: Student-t noise
- ```markdowns/03-25-26-experiments/result_analysis.ipynb```: result visualizations + figures/tables replications for Section 6.1 of the paper.
...
1. **Section 6.1 (simulation)**: Navigate any of the ```project/x04_bo_experiments_*``` folders: we divide all $61$ functions into three batches and run BO in parallel for computational convieniece.
   ...
   - There will be 3 separate pickle files in ```models/X04_````
   - To visualize these results and replicate all the tables and figures in the paper , navigate to ```markdowns/03-25-26-experiments/result_analysis.ipynb``` and run all the cells.
