...
class BayesianOptimizer:
    def __init__(
        ...
        alpha: numpy array, or "hw", "hw2", or "md"
        ...
        if isinstance(alpha, np.ndarray):
            ...
            self.alpha_lb, self.alpha_ub = 1e-2, 1
            if alpha == "hw":
                self.alpha = np.ones(self.n_iter)
                self.cummulative_mse = 0 
            # elif alpha == "hw2":
            #     #alpha_init = self.get_hw2_alpha()
            #     #self.alpha = np.ones(self.n_iter) * alpha_init
            #     self.alpha = np.ones(self.n_iter)
            #     self.cummulative_mse = 0 
            elif "md" in alpha:
                if md_alpha is None:
                    raise ValueError("Users have specified Miller & Dunson coarsed posterior, need to specify md_alpha as well.")
                self.alpha = np.array([md_alpha/(md_alpha+i) for i in range(self.n_input, self.n_input+n_iter)])
            else:
                raise ValueError("Unrecognized alpha arguments!")
            self.alpha_type = alpha

        self.gp.current_alpha = self.alpha[0]
        self._signal_kernel = None
        self._noise_var = None

    def get_hw_alpha(self):
        """Proposed Approach to Compute Alpha"""
        pass

    # def get_hw2_alpha(self):
    #     """Second Approch to get HW alpha"""
    #     X_train = self.X[: self.end_indx]
    #     y_train = self.Y[: self.end_indx].ravel()

    #     # Fit diagnostic GP with alpha=1 to assess dispersion
    #     prev_alpha = getattr(self.gp, 'current_alpha', 1.0)
    #     self.gp.current_alpha = 1.0
    #     self.gp.fit(X_train, y_train)

    #     # Read off noise variance and compute residual variance
    #     summed = self.gp.kernel_
    #     noise_var = float(summed.k2.noise_level)
    #     mu_train, std_train = self.gp.predict(X_train, return_std=True)
    #     #resid = y_train - mu_train
    #     var_f_train = std_train**2
    #     # compute w hat
    #     n_train = X_train.shape[0]
    #     w_hat = np.sqrt((2*np.sum(var_f_train)+noise_var*n_train)/(np.sum(var_f_train) + self.cummulative_mse))
    #     # Clip for stability
    #     lo, hi = self.alpha_lb, self.alpha_ub
    #     w_hat = float(np.clip(w_hat, lo, hi))
    #     # restore previous alpha (caller will set the chosen alpha)
    #     self.gp.current_alpha = prev_alpha
    #     return w_hat
    ...
    def propose_location(self):
        ...
        for x in X_cand:
            ...
            # elif self.alpha_type == "hw2":
            #     temp_alpha = self.get_hw2_alpha()
            else:
                temp_alpha = self.alpha[self.current_indx]
            if self.g[self.current_indx] == 1:
                acq = lambda x: -self.expected_improvement(x, temp_alpha)
            elif self.g[self.current_indx] ==0:
                acq = lambda x: -self.probability_improvement(x, temp_alpha)
            elif self.g[self.current_indx] == 2:
                acq = lambda x: -self.power_2_expected_improvement(x, temp_alpha)
            elif self.g[self.current_indx] == 3:
                acq = lambda x : -self.power_3_expected_improvement(x, temp_alpha)
            else:       
                acq = lambda x: -self.general_expected_improvement(x, temp_alpha)
    ...
    def simulate_optimization(
        ...
        for i in tqdm(range(self.n_iter)):
            ...
            # if self.alpha_type == "hw2":
            #     y_next_pred = self.gp.predict(np.array(x_next).reshape(1,-1), return_std=False)[0]
            #     self.cummulative_mse += (y_next-y_next_pred)**2
            if max_val is not None:
