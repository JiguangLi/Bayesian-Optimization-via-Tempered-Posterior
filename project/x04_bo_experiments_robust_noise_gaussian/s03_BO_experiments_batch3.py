...
KNOWN_MAX_VALUES = {
    # Current hartmann3_3d returns -f_native.
    # Standard Hartmann3 minimum is approximately -3.86278,
    # so the current maximization value is +3.86278.
    ("hartmann3", 3): 3.8627797869493365,

    # Current hartmann4_4d returns -(1.1 - outer) / 0.839.
    # This is the Picheny/SFU rescaled 4D Hartmann maximization scale.
    # Computed from the exact implementation.
    ("hartmann4", 4): 3.134494141221212,

    # Current hartmann6_6d returns -f_native where
    # f_native = -(2.58 + outer) / 1.94.
    # Standard Hartmann6 has outer maximum about 3.3223680114.
    ("hartmann6", 6): (2.58 + 3.32236801141551) / 1.94,

    # Current michalewicz_nd returns -f_native, so maxima are
    # the positive versions of the usual minima.
    ("michalewicz", 5): 4.687658,
    ("michalewicz", 10): 9.6601517,

    # Current shekel_4d returns -f_native, i.e. positive Shekel-10 value.
    ("shekel", 4): 10.536443153482534,
}



def get_true_optimum(func_name, func_info, dim):
    """
    Return the true maximum value in the same scale as func_info["func"].

    Supports:
      - scalar optima, e.g. 0.0
      - dimension-specific dicts, e.g. {5: ..., 10: ...}
      - fallback to KNOWN_MAX_VALUES
    """
    dim = int(dim)
    opt = func_info.get("optimum", None)

    if isinstance(opt, dict):
        if dim not in opt:
            raise KeyError(f"No optimum registered for {func_name}, dim={dim}.")
        return float(opt[dim])

    if opt is not None:
        return float(opt)

    key = (func_name, dim)
    if key in KNOWN_MAX_VALUES:
        return float(KNOWN_MAX_VALUES[key])

    raise ValueError(
        f"Missing true optimum for function={func_name}, dim={dim}. "
        "Add it to FUNCTIONS_Batch3 or KNOWN_MAX_VALUES."
    )
...
FUNCTIONS_Batch3 = {
    ...
    'michalewicz': {
    'func': michalewicz_nd,
    'optimum': {5: 4.687658, 10: 9.6601517},
    'description': 'nD; highly multimodal (controlled by m); current return is unanchored maximization',
    'dim': None,
    },
    'hartmann3': {
        'func': hartmann3_3d,
        'optimum': 3.8627797869493365,
        'description': '3D only; multimodal; native domain [0,1]^3; current return is unanchored maximization',
        'dim': 3,
    },
    'hartmann4': {
        'func': hartmann4_4d,
        'optimum': 3.134494141221212,
        'description': '4D only; Picheny/SFU rescaled Hartmann; current return is unanchored maximization',
        'dim': 4,
    },
    'hartmann6': {
        'func': hartmann6_6d,
        'optimum': (2.58 + 3.32236801141551) / 1.94,
        'description': '6D only; SFU/Picheny scaled Hartmann; current return is unanchored maximization',
        'dim': 6,
    },
    'shekel': {
        'func': shekel_4d,
        'optimum': 10.536443153482534,
        'description': '4D only; Shekel m=10; current return is unanchored maximization',
        'dim': 4,
    }
}
...
def run_single_optimization(
    ...
    # dim = config["dim"]
    dim = config["dim"]
    true_optimum = get_true_optimum(func_name, func_info, dim)
